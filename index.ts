...
const generateDataForDateRange = (startDate, numberOfDays, monthName) => {
  ...
  for (
    ...
    writeDataToCSV(
      `${monthName}/${filenameDate}/NPCI_DATA/UPIMERCHANTRAWDATAACQSBM${filenameDate}.csv`,
      npciHeaders,
      () => generateNpciData(ROW_DATA, npciFormattedDate, commonData)
    );
    writeDataToCSV(
      `${monthName}/${filenameDate}/SWITCH_DATA/switch_txns_${filenameDate}.csv`,
      switchHeaders,
      () => generateSwitchData(ROW_DATA, switchFormattedDate, commonData)
    );
    writeDataToCSV(
      `${monthName}/${filenameDate}/CBS_DATA/cbs_txns_${filenameDate}.csv`,
      cbsHeaders,
      () => generateCbsData(ROW_DATA, formattedDate, commonData)
    );
    writeDataToCSV(
      `${monthName}/${filenameDate}/ADJUSTMENT/ADJUSTMENT${filenameDate}.csv`,
      adjustHeaders,
      () => generateAdjustmentData(ROW_DATA, cbsFormattedDate, commonData)
    );
  }
};

// Usage example 
const startDate = new Date(2024, 5, 21); // June 1, 2024
